...
- **HyruleLink reads inventory bytes and writes items.** That is why it is not
...
- **No leading-slash URLs** in `web/`: it runs under `/hyrulelink/` too. A test
  enforces it.
...
live at https://www.billogna.lol/hyrulelink/ and https://hyrulelink.billogna.lol:
...
HyruleLink's own blocks, and never another service. Line endings are LF in
git; `core.autocrlf` checks worktrees out CRLF, and `deploy/deploy.sh` must
stay LF on disk. UTF-8 without a BOM.
